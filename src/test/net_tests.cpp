...
BOOST_AUTO_TEST_CASE(v2transport_test)
{
    ...
    // Test SendDecoyPacket functionality
    {
        for (int i = 0; i < 10; ++i) {
            V2TransportTester tester(m_rng, true);
            FastRandomContext decoy_rng;

            // Before handshake completes, SendDecoyPacket should fail
            BOOST_CHECK(!tester.GetTransport().SendDecoyPacket(32, decoy_rng));
            ...
            for (size_t decoy_size : {1, 32, 128, 256}) {
                // Send a decoy packet
                BOOST_CHECK(tester.GetTransport().SendDecoyPacket(decoy_size, decoy_rng));
            ...
            BOOST_CHECK(tester.GetTransport().SendDecoyPacket(64, decoy_rng));
            ret = tester.Interact();
            BOOST_REQUIRE(ret && ret->empty());
            size_t received_size = tester.ReceiveDecoyPacket();
